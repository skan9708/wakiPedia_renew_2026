...
export function WineRegistMobile() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const payload = { nameEng, nameKr, region, breed, type, hasImage: !!image };
    console.log('create wine', payload);
    alert('로컬 데모: 콘솔에 생성 페이로드 출력');
  };
    ...
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <section className="mx-auto w-full max-w-mobile px-4">
        ...
        <div className="card p-4 space-y-3">
          ...
          <label className="block">
            ...
            <input value={region} onChange={(e)=>setRegion(e.target.value)} className="w-full border border-border rounded-lg px-3 py-2" placeholder="France" />
          </label>
          <label className="block">
            <span className="block text-sm text-muted mb-1">품종</span>
            <input value={breed} onChange={(e)=>setBreed(e.target.value)} className="w-full border border-border rounded-lg px-3 py-2" placeholder="Merlot, Cabernet Franc" />
          </label>
        </div>
      </section>
      ...
      <div className="fixed bottom-16 left-0 right-0 bg-white/95 backdrop-blur border-t border-border">
        <div className="mx-auto w-full max-w-mobile px-4 py-2.5">
          <button className="w-full py-3 rounded-2xl bg-accent text-white" type="submit">등록하기</button>
        </div>
      </div>
      <div className="h-24" />
    </form>
